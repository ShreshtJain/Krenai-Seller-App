...
class SalesAvgOrderAdapter(val context: Context?, val itemList: ArrayList<DataSalesOverTime>) :
    RecyclerView.Adapter<SalesAvgOrderAdapter.SalesAvgOrderViewHolder>() {
    private val viewPool = RecyclerView.RecycledViewPool()
    private var sharedPreferencesStatus: SharedPreferences? = null
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): SalesAvgOrderAdapter.SalesAvgOrderViewHolder {
        ...
        return SalesAvgOrderViewHolder(itemView)
    }
    ...
    override fun onBindViewHolder(
        holder: SalesAvgOrderAdapter.SalesAvgOrderViewHolder,
        position: Int
    ) {
        val data = itemList[position]
        holder.date.text = data.date
        if(data.Return.toUpperCase().equals("NULL") || data.Return.equals(""))
        {
            holder.Return.text = "₹ " + "0.00"
        }
        else {
    ...
    class SalesAvgOrderViewHolder(view: View) : RecyclerView.ViewHolder(view) {
