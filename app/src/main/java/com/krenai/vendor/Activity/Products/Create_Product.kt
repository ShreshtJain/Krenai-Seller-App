...
class Create_Product: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_create__product)
        sharedPreferencesStatus = getSharedPreferences(
            "loginStatus",
            Context.MODE_PRIVATE
        )
        val editor = sharedPreferencesStatus!!.edit()
        editor.putString("variantJSONArray",JSONArray("[{}]").toString())
        editor.apply()
        ...
        if(sharedPreferencesStatus?.getString("ProductDescription",null)==null)
        {
            desc.setText("")
        }
        else
        ...
        if(sharedPreferencesStatus?.getString("ProductName",null)==null)
        {
            title.setText("")
        }
        else{
        ...
        if(sharedPreferencesStatus?.getString("ProductCategory",null)==null)
        {
           brand.setText("")
        }
        else{
        ...
        if(filePath != null)
        {
            ...
            val urlTask = uploadTask?.continueWithTask(Continuation<UploadTask.TaskSnapshot, Task<Uri>> { task ->
                ...
                if (task.isSuccessful) {
                    ...
                    val url="https://www.outsourcecto.com/api/v3/stores/medias/bulk"
    ...
    {
        val Object=arr.getJSONObject(0)
        ...
        val json="{\"createdDate\":null,\"updatedDate\":null,\"isFlag\":1,\"createdBy\":null,\"updatedBy\":null," +
                "\"state\":{\"id\":1,\"description\":\"ACTIVE\",\"type\":\"Product\"},\"name\":\"${title.text}\",\"description\":\"${desc.text}\"," +
    ...
    fun update(Jsonarray: JSONArray) {
        ...
        val jsonParams2 ="{\"createdDate\": null,\"updatedDate\": null,\"isFlag\": 1,\"createdBy\": null," +
                "\"updatedBy\": null,\"state\": {\"id\": 1,\"description\": \"ACTIVE\",\"type\":\"Product\"}," +
...
//                Toast.makeText(this@Order_Details, it.message, Toast.LENGTH_SHORT).show()
   ...
   private fun setUpVariantRecycler(jsonArray: JSONArray)
    {
    ...
    fun generate(sets: ArrayList<ArrayList<String>>) {
        var variantList= arrayListOf<DataVariantMatrix>()
        var productListing=JSONArray()
        var list:ArrayList<List<String>> =arrayListOf()
        var solutions = 1
        run {
            var i = 0
            while (i < sets.size) {
        ...
        for(i in 0 until list.size)
        {
            ...
            for (i in 0 until data.size) {
                s += data[i] + ","
                variantValues.put(data[i])
            }
            s=s.dropLast(1)
            var Model=DataVariantMatrix(s,"","","","","","","","","","")
